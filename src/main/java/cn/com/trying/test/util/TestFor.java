...
//        testWhile();
        ...
        List<Apple> greenList = filterApples(list, TestFor::isGreenApple);
        List<Apple> heavyList = filterApples(list, TestFor::isHeavyApple);
    ...
    */
    public static List<Apple> getCopyAppleList(int count,String color){
        ...
        Apple p = new Apple();
        List<Apple>  list = Lists.newArrayList();
        //这种方式要比iterate快一些，因为减少了基础类型的装箱拆箱操作
...
//        Stream.iterate(1,i -> i+1).limit(count).forEach(i -> {
//            Apple apple =p;
//            apple.setColor(color);
//            list.add(apple);
//        });
